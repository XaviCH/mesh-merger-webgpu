...
export class MetaScene {
    ...
    generateVBO() {
        ...
        this.VBOBufferLayout = {
            arrayStride: 12+12+this.mesh.meshes.length*3*4*2,
            attributes: [
                {
                format: "float32x3",
                offset: 0,
                ...
                ...this.mesh.meshes.map(mesh => {
                    return {
                        format: "float32x3",
                        offset: 12+this.mesh.meshes.indexOf(mesh)*3*4,
                        shaderLocation: this.mesh.meshes.indexOf(mesh)+1,
                    }
                }),
                {
                format: "float32x3",
                offset: 12+this.mesh.meshes.length*3*4,
                shaderLocation: this.mesh.meshes.length+1,
                },
                ...this.mesh.meshes.map(mesh => {
                    return {
                        format: "float32x3",
                        offset: 12+this.mesh.meshes.length*3*4+this.mesh.meshes.indexOf(mesh)*3*4+12,
                        shaderLocation: this.mesh.meshes.length+this.mesh.meshes.indexOf(mesh)+2,
                    }
                }),
            ],
        };
    ...
    generateUniforms() {
        ...
        this.scalars = new Float32Array(this.mesh.meshes.map(_ => 0));
